...
function dfs(cur,next){
	...
	const predecessor = cur.right === null ? cur.left : cur.right
	const successor = next === null ? null : next.left === null ? next.right : next.left
	dfs(predecessor, successor)
}
```
